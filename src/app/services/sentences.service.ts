...
export class SentencesService {
  ...
  moveUp(id: string): Observable<Sentence[]> {
    return this.auth.getAuthHeaders().pipe(
      switchMap(headers =>
        this.http.put<Sentence[]>(`${this.apiUrl}/${id}/move-up`, {}, { headers })
      ),
      catchError(error => {
  ...
  moveDown(id: string): Observable<Sentence[]> {
    return this.auth.getAuthHeaders().pipe(
      switchMap(headers =>
        this.http.put<Sentence[]>(`${this.apiUrl}/${id}/move-down`, {}, { headers })
      ),
      catchError(error => {
        console.error("Nie możne przesunąc w górę", error);
        return of([])
  ...
  return this.auth.getAuthHeaders().pipe(
    switchMap(headers =>
      this.http.post<Sentence>(`${this.apiUrl}/single`, { content }, { headers })
        .pipe(
